...
// intitialize the json file and start 
initialize = () => {
  fetch(Questions)
    .then(res => {
      return res.json();
...
getNewQuestion = () => {
  if (availableQuesions.length === 0 || questionCounter >= MAX_QUESTIONS) {
    ...
    return window.location.replace("../pages/quizover.html");
  }
  questionCounter++;
  progressText.innerText = `Question ${questionCounter} /   ${MAX_QUESTIONS}`;
  //Update the progress bar( Koffi, see if you can fix this, its not working yet )
